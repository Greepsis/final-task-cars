...
public class UserCommentControllerTest {
    ...
    @Test
    @Transactional
    public void createCorrectComment_thenStatus_isCreated() throws Exception {
        Ad ad = createTestAd();
        HashMap<String,Object> addCommentDto = new HashMap<>();
        addCommentDto.put("id",ad.getId());
        addCommentDto.put("comment","Cool car");
        mockMvc.perform(
    ...
    @Test
    @Transactional
    public void correctUpdateComment_thenStatus_isOk() throws Exception {
        Ad ad = createTestAd();
        Comment comment = createTestComment(ad.getId());
        HashMap<String,Object> addCommentDto = new HashMap<>();
        addCommentDto.put("id",comment.getId());
        addCommentDto.put("comment","Nice car");
        mockMvc.perform(
                ...
                .andExpect(jsonPath("$.data.comment").value(addCommentDto.get("comment")));
    }

    @Test
    @Transactional
    public void correctDeleteComment_thenStatus_isOk() throws Exception {
        Ad ad = createTestAd();
        Comment comment = createTestComment(ad.getId());
        mockMvc.perform(
                delete("/api/user/comment/{id}",comment.getId())
                        .header("Authorization","Bearer " + getUserToken())
                        .contentType(MediaType.APPLICATION_JSON)
        )
                .andDo(print())
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data").value("Comment:" + comment.getId() + " can be deleted"));
    }
    ...
    private Comment createTestComment(Integer adId){
        User user = userRepository.getUserByEmail("user15@gmail.com");
        Ad ad = adRepository.getById(adId);
        Comment comment = new Comment(null,ad,user,"Cool car",LocalDateTime.now());
